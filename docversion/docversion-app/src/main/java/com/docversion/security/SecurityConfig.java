    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        http
            .authorizeHttpRequests(auth -> auth
                ...
                // 08/19 - DLP 규칙 관리(RD-SRS-5.4, 5.6)는 전역 설정이므로 관리자 전용.
                //   문서별 검사 결과 조회(/api/documents/**/dlp)는 문서에 귀속되므로
                //   여기에 두지 않고 DocumentAccessPolicy가 객체 단위로 판단한다.
                .requestMatchers("/api/dlp/**").hasRole("ADMIN")
